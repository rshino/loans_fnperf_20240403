...
, FP((1-coalesce(mapinel_pct,0)),1)		as "MAP Elig.%"
 ...
 -- begin grouping sections
  ...
  /*_BYBAL_  ,case when bal_seg = '0' then 'All Conf. Balances' else bal_seg end as "Cohort Balance" _BYBAL_*/
  /*_BYBALP_ ,case when bal_seg = '0' then 'All Conf. Balances' else bal_seg end as "Cohort Balance"  _BYBALP_*/
  /*_BYBSTP_ ,case when bal_seg = '0' then 'All Conf. Balances' else bal_seg end as "Cohort Balance"  _BYBSTP_*/
  /*_BYFRST_ ,first_flag 	      	   	      		     as "Cohort 1st Home Buyer" _BYFRST_*/
  /*_BY2HOM_ ,second_home 					     as "Cohort 2nd Home" _BY2HOM_*/
...
,format(CT_COHORT,0)					as "Cohort Orig #"
,format(OBAL_COHORT, 0) 				as "Cohort Orig $"
 -- end cohort loans info


/*_BYAGE_ 
	  , loan_age					as "Loan Age"
	  , concat(format(cpr*100,2),'%')  		as "CPR"
	  , concat(format(D30DDPCT*100,2),'%') 		as "30DD%"	  
	  , concat(format(D60DDPCT*100,2),'%')  	as "60DD%"	  
	  , concat(format(D90PLUSPCT*100,2),'%')  	as "90+DD%"	  
	  , concat(format(FCPCT*100,2),'%')  		as "FC%"	  

	  , concat(format(
	    sum(case when loan_age is not null then defaulted_upb end) 
	    over (partition by loan_term
_BYAGE_*/
	  	  #BYAGE# /*_BYVINT_ ,vintage _BYVINT_*/
	  	  #BYAGE# /*_BYFRST_ ,first_flag _BYFRST_*/
	  	  #BYAGE# /*_BY2HOM_ ,second_home _BY2HOM_*/
	  	  #BYAGE# /*_BYMI_   ,mi_flag _BYMI_*/
	  	  #BYAGE# /*_BYHBAL_ ,hbal _BYHBAL_*/
	  	  #BYAGE# /*_BYCHAN_ ,channel _BYCHAN_*/
	  	  #BYAGE# /*_BYST_   ,state _BYST_*/
	  	  #BYAGE# /*_BYBAL_  ,bal_seg _BYBAL_*/
	  	  #BYAGE# /*_BYBALP_  ,bal_seg _BYBALP_*/
	  	  #BYAGE# /*_BYBSTP_  ,bal_seg _BYBSTP_*/
/*_BYAGE_		  
...
/*_BYAGE_
	  ...
	  , format(loan_ct,0)				as "Loans"
	  , format(current_upb,0) 			as "Curr Bal"
	  , format(orig_upb,0) 				as "Orig Bal"
	  , concat(format(smm*100,6),'%') 		as "SMM"
	  , format(sched_bal,0) 			as "Sched Bal"
	  , format(unsched_prin,0) 			as "Unsched Prin"
	  , format(defaulted_upb,0) 			as "Mo Defaults"
	  , format(defaulted_cnt,0) 			as "Mo Def Cnt"
	  , format(coalesce(MO_NET_LOSS,0),0) 		as "Mo Net Loss"	  
	  , format(coalesce(loss_cnt,0),0) 		as "Mo Loss Cnt"	  
	  , format(sum(case when loan_age is not null then defaulted_upb end) over (partition by loan_term
_BYAGE_*/
	  	  #BYAGE# /*_BYVINT_ ,vintage _BYVINT_*/
		  #BYAGE# /*_BYFRST_ ,first_flag _BYFRST_*/
		  #BYAGE# /*_BY2HOM_ ,second_home _BY2HOM_*/
...
/*_BYAGE_
	  ...
	  , format(sum(case when loan_age is not null then MO_NET_LOSS end) over (partition by loan_term
_BYAGE_*/
	  	  #BYAGE# /*_BYVINT_ ,vintage _BYVINT_*/
		  #BYAGE# /*_BYFRST_ ,first_flag _BYFRST_*/
		  #BYAGE# /*_BY2HOM_ ,second_home _BY2HOM_*/
