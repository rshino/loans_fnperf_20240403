...
shellscript<-'../00util/run_sql_v24.sh'
sql<-'fn_v085.sql'
# speeds by loan size
...
#params<-'TERM=360 VINT=2018 ELIG BYAGE BYCHAN BY2HOM MINCOUNT=100'
#params<-'TERM=360 VINT=2019 ELIG BYAGE BYCHAN BY2HOM MINCOUNT=100'
...
#params<-'TERM=360 VINT=2023 BYAGE BYCHAN BY2HOM'
...
params<-'F_VINT=2022 F_TERM=180 F_ELIG BY_AGE BY_ST MINCOUNT=100'
...
ggplot(cohortcf,aes(x=Loan.Age, y=CPR
                     ,linetype=factor(Cohort.Channel)
                     ,color=factor(Cohort.2nd.Home)
                    ,group=interaction(Cohort.Channel,Cohort.2nd.Home)))+
geom_line()+
  ggtitle(paste0(title,"\n",params))+
  xlab(xlab)+
  ylab(ylab)+
  labs(color=factor)+
  labs(linetype=factor1)+
